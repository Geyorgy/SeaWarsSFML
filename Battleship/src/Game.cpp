...
void Game::render()
{
	m_Window.clear(Color(255, 254, 240)); //out map color
...
void Game::renderPlayer(Player* p, int numPlayer)
{
	...
	if (numPlayer == 1)
		p->m_PlayerName.setString("Captain 1");
	else if (numPlayer == 2)
		p->m_PlayerName.setString("Captain 2");

	if (p->getIsItComp())
		p->m_PlayerName.setString("Computer");

	p->m_PlayerName.setPosition(p->m_Camera.getCenter().x - 45, 10);
